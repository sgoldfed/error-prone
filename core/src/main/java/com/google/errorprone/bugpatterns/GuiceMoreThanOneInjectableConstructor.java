...
explanation = "Having more than one injectable constructor will throw a runtime error in Guice",
category = GUICE, severity = ERROR, maturity = EXPERIMENTAL)
public class GuiceMoreThanOneInjectableConstructor extends DescribingMatcher<MethodTree>  {
  ...
  public Description describe(MethodTree methodTree, VisitorState state) {
          for (AnnotationTree annotation : methodTree.getModifiers().getAnnotations()){
            if (ASTHelpers.getSymbol(annotation).equals
                  (state.getSymbolFromString(GUICE_INJECT_ANNOTATION)) 
                || ASTHelpers.getSymbol(annotation).equals
  ...
  public static class Scanner extends com.google.errorprone.Scanner {
    public DescribingMatcher<MethodTree> methodMatcher = 
        new GuiceMoreThanOneInjectableConstructor();
