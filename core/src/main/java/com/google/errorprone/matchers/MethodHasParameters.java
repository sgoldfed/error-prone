 ...
 * matcher to a set of nodes. Consider refactoring this code into the base class for better reuse.
 *
...
public class MethodHasParameters extends MultiMatcher<MethodTree, VariableTree> {
  ...
  public boolean matches(MethodTree methodTree, VisitorState state) {
    ...
    return (matchType == ANY && foundMatch)
        || (matchType == ALL && methodTree.getParameters().size() >= 1);
  }
}
